...
func MakeRequest(gid int) (resp string) {
	u := "https://api.csgorun.gg/games/" + strconv.Itoa(gid)
...
func Unparse(resp string) (stroka string, err error) {
	...
	var peopleWin float32
	peopleWin = 0
	//win - stavka
	var peopleLost float32
	peopleLost = 0
	//red deposits

	for _, s := range dat.Data.Bets {
		//fmt.Println(s.Deposit.Amount)
		if s.Withdraw.Amount != 0.0 {
			peopleWin = peopleWin + (s.Withdraw.Amount - s.Deposit.Amount)
		} else if s.Withdraw.Amount == 0.0 {
			peopleLost = peopleLost + s.Deposit.Amount
		}
	}
	...
	_ = fmt.Sprint(dat.Data.Id, ";", summ-withdrawsumm, ";", summ, ";", withdrawsumm, ";", maxk, ";", g, ";", badgame)
	stroka = fmt.Sprint(dat.Data.Id, ";", peopleWin, ";", peopleLost)
	fmt.Println(stroka)
	fmt.Println(g)
	return stroka, nil
}
